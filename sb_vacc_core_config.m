...
function sb_vacc_core_config(this_block)

  % Revision History:
  %
  %   11-Nov-2013  (17:03 hours):
  %     Original code was machine generated by Xilinx's System Generator after parsing
  %     /media/data/jack/ami/ami_correlator/sb_vacc_core.vhd
  %
  %

  this_block.setTopLevelLanguage('VHDL');

  this_block.setEntityName('sb_vacc_core');
  ...
  dout00_port.setType('UFix_12_0');
  dout01_port = this_block.port('dout01');
  dout01_port.setType('UFix_12_0');
  dout11_port = this_block.port('dout11');
  dout11_port.setType('UFix_12_0');
  vld_port = this_block.port('vld');
  vld_port.setType('UFix_1_0');
  ...
  if (this_block.inputTypesKnown)
    ...
    if (this_block.port('din00').width ~= 144);
      this_block.setError('Input data type for port "din00" must have width=144.');
    end

    if (this_block.port('din01').width ~= 144);
      this_block.setError('Input data type for port "din01" must have width=144.');
    end

    if (this_block.port('din11').width ~= 144);
      this_block.setError('Input data type for port "din11" must have width=144.');
    end
  ...
  %    this_block.addFile('');
  %    this_block.addFile('');
  this_block.addFile('sb_vacc_core.vhd');
