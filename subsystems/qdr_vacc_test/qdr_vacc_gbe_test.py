...
if __name__ == '__main__':
    ...
    print 'enabling QDR'
    fpga.write_int('xeng_ctrl', 0)

    print 'disabling 1gbe interface'
    fpga.write_int('xeng_ctrl', 2) #reset

    acc_len = 1024
    dest_ip_str = '10.1.1.1'
    src_ip_str = '10.1.1.3'
    port = 10000
    src_ip = 0

    for ii, val in enumerate(src_ip_str.split('.')):
        src_ip += int(val) << (8*(3-ii))
    dest_ip = 0
    for ii, val in enumerate(dest_ip_str.split('.')):
        dest_ip += int(val) << (8*(3-ii))

    print 'setting destination to %s:%d (%d)'%(dest_ip_str, port, dest_ip)
    fpga.write_int('one_gbe_tx_ip', dest_ip)
    fpga.write_int('one_gbe_tx_port', port)

    print 'setting source to %s:%d (%d)'%(src_ip_str, port, src_ip)
    fpga.tap_start('tap0', 'one_GbE', 0x001122334455, src_ip, port)

    print 'setting accumulation length to %d'%acc_len
    fpga.write_int('acc_len_mi', acc_len-1)

    print 'setting xeng id to 1'
    fpga.write_int('xeng_id', 1)

    print 'enabling 1gbe interface'
    fpga.write_int('xeng_ctrl', 0) #reset off
    fpga.write_int('xeng_ctrl', 4) #gbe enable
